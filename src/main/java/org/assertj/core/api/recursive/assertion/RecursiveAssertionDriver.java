...
import static org.assertj.core.util.introspection.FieldSupport.getFieldType;
...
public class RecursiveAssertionDriver {
  ...
  public List<FieldLocation> assertOverObjectGraph(Predicate<Object> predicate, Object graphNode) {
    ...
    return unmodifiableList(fieldsThatFailedTheAssertion);
  }
  ...
  private boolean nodeMustBeIgnored(Object node, Class<?> nodeType, FieldLocation fieldLocation) {
    return node == null && configuration.getIgnoreAllActualNullFields()
           || isPrimitiveAndPrimitiveTypesAreIgnored(nodeType)
           || isEmptyOptionalWhichAreIgnored(node, nodeType)
  ...
  private boolean isPrimitiveAndPrimitiveTypesAreIgnored(Class<?> nodeType) {
    return nodeType.isPrimitive() && !configuration.shouldAssertOverPrimitiveFields();
  }

  private boolean isEmptyOptionalWhichAreIgnored(Object node, Class<?> nodeType) {
  ...
  private boolean policyDoesNotForbidAssertingOverNode(Class<?> nodeType) {
    boolean policyForbidsAsserting = isContainerWhichAreIgnored(nodeType);
    policyForbidsAsserting = policyForbidsAsserting || isMapWhichAreIgnored(nodeType);
    return !policyForbidsAsserting;
  }

  private boolean isContainerWhichAreIgnored(Class<?> nodeType) {
    boolean isContainer = isCollection(nodeType) || isArray(nodeType);
    return configuration.shouldIgnoreContainer() && isContainer;
  }

  private boolean isMapWhichAreIgnored(Class<?> nodeType) {
    return configuration.shouldIgnoreMap() && isMap(nodeType);
  }

  private void recurseIntoFieldsOfCurrentNode(Predicate<Object> predicate, Object node, Class<?> nodeType,
                                              FieldLocation fieldLocation) {
    if (isSpecialTypeRequiringSpecificHandling(nodeType)) {
  ...
  private boolean isSpecialTypeRequiringSpecificHandling(Class<?> nodeType) {
    return isCollection(nodeType) || isMap(nodeType) || isArray(nodeType);
  }

  private boolean shouldRecurseOverSpecialTypes(Class<?> nodeType) {
    boolean recurseOverCollection = (isCollection(nodeType) || isArray(nodeType))
  ...
  private void recurseIntoCollection(Predicate<Object> predicate, Collection<?> node, FieldLocation fieldLocation) {
    int idx = 0;
    for (Object o : node) {
      assertRecursively(predicate, o, o != null ? o.getClass() : Object.class, fieldLocation.field(format(INDEX_FORMAT, idx)));
      idx++;
    }
  }
  ...
  private boolean nodeShouldBeRecursedInto(Object node) {
    ...
    nodeShouldBeRecursedInto = nodeShouldBeRecursedInto && !nodeIsJavaTypeToSkip(node);
    return nodeShouldBeRecursedInto;
  }

  private boolean nodeIsJavaTypeToSkip(Object node) {
    boolean skipJCLTypes = configuration.shouldSkipJavaLibraryTypeObjects();
  ...
  private void findFieldsOfCurrentNodeAndDoRecursiveCall(Predicate<Object> predicate, Object node, FieldLocation fieldLocation) {
    ...
    namesOfFieldsInNode.stream()
                       .map(name -> tuple(name, EXTRACTION.getSimpleValue(name, node), getFieldType(name, node)))
                       .forEach(tuple -> {
